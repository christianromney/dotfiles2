...
Packages that integrate with narrow AI LLMs from Open AI.
...
#+begin_example
security add-internet-password -A -r http -s api.openai.com -a <username> -w <api-token> -U -l "openai"
#+end_example
...
Load Dall-E for image generation.
...
** gptel
Yet another ChatGPT emacs front-end, but with a nicer UI.
#+begin_src elisp
(use-package! gptel
  :defer t
  :commands (gptel)
