...
CopyFiles.detailed_help = {
    ...
    'DESCRIPTION': """\
        *{command}* copies files between a virtual machine instance
        ...
        machine instance name (e.g., _INSTANCE:\\~/FILE_). To
        denote a local file, do not add a prefix to the file name
        (e.g., _~/FILE_). For example, to copy a remote directory
        to your local host, run:

          $ {command} INSTANCE:~/REMOTE-DIR ~/LOCAL-DIR --zone us-central1-a

        In the above example, ``~/REMOTE-DIR'' from ``INSTANCE'' is copied
        into the _~/LOCAL-DIR_ directory.

        Conversely, files from your local computer can be copied to a
        virtual machine:

          $ {command} ~/LOCAL-FILE-1 ~/LOCAL-FILE-2 INSTANCE:~/REMOTE-DIR --zone us-central1-a
